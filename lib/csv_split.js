...
Splitter.prototype.parse = function(group_name) {
    // store header and target column in Splitter object
    var self = this;
    var parser = csv();
    return parser;
};
...
Splitter.prototype.drain = function(data) {
    ...
    if (this.write_streams[name]) {
        ...
        ws.write(write_csv_header(data.data));
...
var write_csv_header = function(arr) {
    //adding quotes
    var keys = _.keys(arr);
    var quoted_keys = keys.map(function(item) {
        return '\"' + item + '\"';
    });
    return  quoted_keys.toString() + '\n';
};
var write_csv_line = function(arr) {
    //adding quotes
    var values = _.values(arr);
    var quoted_values = values.map(function(item) {
        return '\"' + item + '\"';
    });
    return quoted_values.toString() + '\n';
};
